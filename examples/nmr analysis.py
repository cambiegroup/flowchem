...
async def Analysis(observed_result):
    ...
    while True:
        path = await nmr.run_protocol(
            "1D FLUORINE+",
            {"Number": 128, "AcquisitionTime": 3.2, "RepetitionTime": 2, "PulseAngle": 90},
        )
        ...
        observed_result = observed_result.append(
                pd.DataFrame(peak_normalized_list, index=["SM", "product", "side-P"], columns=[observed_time]).T)

        #result
        print(observed_result)
        # save
        observed_result.to_csv(r'W:\BS-FlowChemistry\People\Wei-Hsin\Spinsolve\export_dataframe_0317_03.csv', header=True)
        plt.figure()
        observed_result.plot()
        plt.legend(loc='best')
        plt.savefig(r"W:\BS-FlowChemistry\People\Wei-Hsin\Spinsolve\export_plot_0317_03.png")
...
async def main():
    observed_time = 0
    observed_result = pd.DataFrame([1, 0, 0], index=["SM", "product", "side-P"], columns=[observed_time]).T
