...
#td do same magic here/folderpath should always be appended
    ...
    def load_and_decode_batch(self):
        ...
        for files in self.experiment_folder.glob("*" + self.file_extension):
            ...
            if one_condition.temperature not in loaded_experiments.keys() :
                ...
                same_T_1: dict = {k: v for k, v in loaded_experiments[one_condition.temperature].__dict__.items() if k not in ["_chromatogram", "_experiment_id"]}
                same_T_2: dict ={k: v for k, v in one_condition.__dict__.items() if k not in ["_chromatogram", "_experiment_id"]}
                ...
                loaded_experiments[f"{one_condition.temperature} {same_T_key_1}"] = loaded_experiments[one_condition.temperature]
    ...
    def plot_all_traces(self, experiments_conditions: dict):
        "Hand dict for experiments_conditions, where key is the label you want in plot and value is one specific experiment condition"
        canvas = None
